    ...
    return HtmlElementView(
      ...
      onPlatformViewCreated: (int viewId) {
        ...
        if (element != null) {
          ...
          String createSvg(String pathData, {String viewBox = "0 0 24 24"}) {
            return '<svg viewBox="$viewBox" fill="white" style="width: 28px; height: 28px;"><path d="$pathData"></path></svg>';
          }

          // PLAY BUTTON
          final playButton = html.Element.tag('media-play-button');
          // Add both Play and Pause icons (Vidstack toggles them automatically based on state if we treat them right,
          // but specifically for manual slot usage, it's safer to just let the button handle logic and we provide generic content
          // OR we trust the browser theme. Since theme failed, we inject SVG.)
          // Actually, 'media-play-button' generally replaces its content with the relevant icon if using 'media-video-layout'.
          // Without it, we must provide the slots.
          playButton.innerHtml = """
            <media-icon type="play" slot="play">${createSvg('M8 5v14l11-7z')}</media-icon>
            <media-icon type="pause" slot="pause">${createSvg('M6 19h4V5H6v14zm8-14v14h4V5h-4z')}</media-icon>
            <media-icon type="replay" slot="replay">${createSvg('M12 5V1L7 6l5 5V7c3.31 0 6 2.69 6 6s-2.69 6-6 6-6-2.69-6-6H4c0 4.42 3.58 8 8 8s8-3.58 8-8-3.58-8-8-8z')}</media-icon>
          """;
          ...
          muteButton.innerHtml = """
            <media-icon type="volume-high" slot="volume-high">${createSvg('M3 9v6h4l5 5V4L7 9H3zm13.5 3c0-1.77-1.02-3.29-2.5-4.03v8.05c1.48-.73 2.5-2.25 2.5-4.02zM14 3.23v2.06c2.89.86 5 3.54 5 6.71s-2.11 5.85-5 6.71v2.06c4.01-.91 7-4.49 7-8.77s-2.99-7.86-7-8.77z')}</media-icon>
            <media-icon type="volume-low" slot="volume-low">${createSvg('M18.5 12c0-1.77-1.02-3.29-2.5-4.03v8.05c1.48-.73 2.5-2.25 2.5-4.02zM5 9v6h4l5 5V4L7 9H5z')}</media-icon>
            <media-icon type="volume-muted" slot="volume-muted">${createSvg('M16.5 12c0-1.77-1.02-3.29-2.5-4.03v2.21l2.45 2.45c.03-.2.05-.41.05-.63zm2.5 0c0 .94-.2 1.82-.54 2.64l1.51 1.51C20.63 14.91 21 13.5 21 12c0-4.28-2.99-7.86-7-8.77v2.06c2.89.86 5 3.54 5 6.71zM4.27 3L3 4.27 7.73 9H3v6h4l5 5v-6.73l4.25 4.25c-.67.52-1.42.93-2.25 1.18v2.06c1.38-.31 2.63-.95 3.69-1.81L19.73 21 21 19.73 4.27 3zM12 4L9.91 6.09 12 8.18V4z')}</media-icon>
          """;
          ...
          fsButton.innerHtml = """
             <media-icon type="fullscreen" slot="enter">${createSvg('M7 14H5v5h5v-2H7v-3zm-2-4h2V7h3V5H5v5zm12 7h-3v2h5v-5h-2v3zM14 5v2h3v3h2V5h-5z')}</media-icon>
             <media-icon type="fullscreen-exit" slot="exit">${createSvg('M5 16h3v3h2v-5H5v2zm3-8H5v2h5V5H8v3zm6 11h2v-3h3v-2h-5v5zm2-11V5h-2v5h5V8h-3z')}</media-icon>
          """;
